...
public static class LensWrapperExtensions
{
    ...
    ///        .Update(book => book with { Title = newTitle }); // This method.
    /// </code>
    /// <para>
    /// Typical usages include:
    /// <list type="bullet">
    /// <item>Edit a value: <c>.Update(book => book with { Title = newTitle });</c></item>
    /// <item>Add an item to a collection: <c>.Update(books => [.. books, secondBook]);</c></item>
    /// <item>Remove an item from a collection: <c>.Update(books => books.Where(book => book.ISDN != bookISDN).ToArray());</c></item>
    /// </list>
    /// </para>
    /// </summary>
    ...
    /// <returns></returns>
    public static TWhole Update<TWhole, TPart>(this LensWrapper<TWhole, TPart> wrapper, Func<TPart, TPart> transform)
